...
function exportToExcel($conn) {
    ...
    $sheet->setCellValue('D1', 'Precio por Unidad');
    $sheet->setCellValue('E1', 'Total Cajas');
    $sheet->setCellValue('F1', 'Stock en Tienda');
    $sheet->setCellValue('G1', 'Stock en Almacén');
    $sheet->setCellValue('H1', 'Stock Total');
    $sheet->setCellValue('I1', 'Categoría');

    // Obtener los datos de la base de datos
    $result = $conn->query("SELECT productos_cajas.*, categorias.nombre_categoria FROM productos_cajas LEFT JOIN categorias ON productos_cajas.categoria_id = categorias.id");
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $sheet->setCellValue('D' . $rowNumber, $row['precio_unidad']);
            $sheet->setCellValue('E' . $rowNumber, $row['total_cajas']);
            $sheet->setCellValue('F' . $rowNumber, $row['stock_tienda']);
            $sheet->setCellValue('G' . $rowNumber, $row['stock_almacen']);
            $sheet->setCellValue('H' . $rowNumber, $row['stock_tienda'] + $row['stock_almacen']);
    ...
    $filename = 'Inventario_Productos_Por_Cajas.xlsx';
...
<head>
    ...
    <title>Inventario por Cajas</title>
...
<body>
    ...
    <div class="content">
        <h2>Inventario de Productos por Cajas</h2>
        <a href="inventario_cajas.php?export=excel" class="btn btn-success">Exportar a Excel</a>
        <form action="importar_inventario_cajas.php" method="post" enctype="multipart/form-data">
            <div class="form-group">
                <label for="import-file-cajas">Archivo Excel:</label>
                <div class="input-group">
                    <input type="file" class="form-control-file" id="import-file-cajas" name="import_file" accept=".xlsx, .xls" required>
                    <div class="input-group-append">
                        <a class="btn btn-secondary" href="descargar_plantilla_cajas.php">Descargar Plantilla</a>
                    </div>
                </div>
            </div>
            ...
            <tbody>
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<td>" . htmlspecialchars(number_format($row['precio_unidad'], 2)) . "</td>";
                        echo "<td>" . htmlspecialchars($row['total_cajas']) . "</td>";
                        echo "<td>" . htmlspecialchars($row['stock_tienda']) . "</td>";
                        echo "<td>" . htmlspecialchars($row['stock_almacen']) . "</td>";
                        echo "<td>" . htmlspecialchars($row['stock_tienda'] + $row['stock_almacen']) . "</td>";
